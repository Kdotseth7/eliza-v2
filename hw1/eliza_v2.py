...
# Set the level of logger to DEBUG to print DEBUG statements
...
class Eliza:
    ...
    def decontracted(self, word_list):
        """Convert contractions like "can't" into "can not"""
        words = [contractions.fix(w) for w in word_list if w]
        return words
    
    def spell_check(self, word_list):
        """Corrects the spelling of the words in the input list"""
        corrected_words = []
    ...
    def respond(self, text):
        ...
        words = self.decontracted(words)
        log.debug('After resolving contractions: %s', words)
        
        # Spell check
        words = self.spell_check(words)
        log.debug('After spell check: %s', words)
